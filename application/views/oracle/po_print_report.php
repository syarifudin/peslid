...
<head>
   ...
   <body>
	<?php
			...
			foreach($data_delivery_to as $del);
			$delivery=$del['delivery_name']." | ".$del['delivery_add']. " |".$del['delivery_city'];
				...
				<?php
					...
					if ($row['BUYER'] == 1111){$buyer_name = 'Puji Yastuti';} 
					...
					if ($row['BUYER'] == 1272){$buyer_name = 'Desy Nanda';}
					
					if ($row['TERMS_ID'] == 10000){$terms_pay = 'C30M10 (By TTR on 10th of next month after shipment)';}
					if ($row['TERMS_ID'] == 10001){$terms_pay = 'C30M30(End of Month Cutoff Next End of Month Payment)';}
					if ($row['TERMS_ID'] == 10023){$terms_pay = 'Net 30 (Payment During 30 Dates from Receives date Of payment request)';}
					if ($row['TERMS_ID'] == 10041){$terms_pay = 'Net 60 (Payment During 60 Dates from Receives date Of payment request)';}
					if ($row['TERMS_ID'] == 10080){$terms_pay = 'Net 14 (Payment During 15 Dates from Receives date Of payment request)';}
					if ($row['TERMS_ID'] == 10081){$terms_pay = 'Net 105 (Payment During 105 Dates from Receives date Of payment request)';} 
					if ($row['TERMS_ID'] == 10025){$terms_pay = 'C30M15 (End Of Month Cutoff Next Month 15th Payment)';}
			...
			?>
					...
					<thead>
					<tr>
						<th><div><font size="1" face="Calibri" color="#A9A9A9">(<?=$row['PO_NUM'];?>)</font><div>No.</th>
						<th>Model Number</th>
