<?php class Po_model extends CI_Model {
	...
	public function save_po_number($number)
	{
	...
	$query=$this->db->query("select * from openquery(qad_connect,'select ad_name,ad_line1,ad_line2,ad_line3,
	ad_city,ad_attn,ad_phone,ad_country from pub.ad_mstr where ad_addr=''$kode_supplier''')");
	return  $query->result_array();
	}
	...
	public function get_csv($getcsv){
	  $po_ams=$getcsv;
	$this->db->select('*')
			  ->from('open_po as p')
			  ->join('pc_po_number as pc','pc.pc_order_number=p.ams_po')
	...
	$du_date[]=$due;
			...
			$due=$row['ddate'];
			$du =date('Y-m-d',strtotime($due));
			$query = $this->db->query("update open_so_detil set due_date_so='$du'
			where so_number='$ams'");
		}
	  }	
	}
	...
	$query=$this->db->query("select  * from openquery(qad_train,
							'select top 1 * from pub.pc_mstr where pc_part=''FCL22D+FCL32D''')");
	...
	$query = $this->db->query("select top 1* from openquery(qad_train,
	 ...
	 $query = $this->db->query("select * from open_po as o ,eta as e, open_po_detil as po ,
								pc_consignee p where 
								o.ams_po=po.ams_po_det and e.ams_=o.ams_po and o.rev=po.rev_ 
								and	p.global_code=o.cnee and ams_po='$ams_po' and o.rev='$st' ");
		return $query->result_array();
		//and dest.city_code=o.dest
