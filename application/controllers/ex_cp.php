...
class Ex extends CI_Controller {
	...
	else
	{
		...
		for($i=1;$i<=$counter2;$i++){
					...
					$number['pc_order_number']=$no=$get_item=substr($baris2[$i],51,25);
					...
					foreach($cek_que as $cek);
						...
						foreach($sup['dlv_tt'] as $nomor);
							...
							$cus_po=substr($nomor['ams_po'],0,7);
							 $file_save='./pdf_po/PO'.$kode_supplier.$nomor['po_number']."-".trim($cus_po)."_".date('dmy')."1".$nomor['rev'].'.pdf';
